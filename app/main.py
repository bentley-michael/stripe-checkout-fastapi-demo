...
def _inline_rl_basic(payload: Dict[str, Any]) -> bytes:
    ...
    c.setFont("Helvetica", 9)
    c.drawRightString(w - 0.75*_inch, h - 0.35*_inch, datetime.utcnow().strftime("Generated: %Y-%m-%d %H:%M UTC"))

    # Basics
    y = h - 1.35*_inch
    c.setFont("Helvetica-Bold", 12); c.drawString(0.75*_inch, y, "Shipment Details"); y -= 0.25*_inch
    c.setFont("Helvetica", 10)
    kv = [
    ...
    for k, v in kv:
        c.drawString(0.75*_inch, y, f"{k}: {v}"); y -= 0.18*_inch

    # Rates
    y -= 0.08*_inch
    c.setFont("Helvetica-Bold", 12); c.drawString(0.75*_inch, y, "Rates"); y -= 0.22*_inch
    c.setFont("Helvetica", 10)
    base = float(payload.get("base_rate",0.0))*100.0
    addl = float(payload.get("tariff_rate",0.0))*100.0
    c.drawString(0.75*_inch, y, f"Base Rate (HTS/TARIC): {base:.2f}%"); y -= 0.18*_inch
    c.drawString(0.75*_inch, y, f"Additional Tariff: {addl:.2f}%"); y -= 0.22*_inch

    # Totals
    c.setFont("Helvetica-Bold", 12); c.drawString(0.75*_inch, y, "Landed Cost Summary (USD)"); y -= 0.22*_inch
    c.setFont("Helvetica", 10)
    ...
    for k, key in [
        ...
        c.drawString(0.75*_inch, y, f"{k}: ${val:,.2f}"); y -= 0.18*_inch

    # Scenarios (if any)
    S = payload.get("scenarios", []) or []
    if S:
        y -= 0.08*_inch
        c.setFont("Helvetica-Bold", 12); c.drawString(0.75*_inch, y, "Scenarios"); y -= 0.18*_inch
        c.setFont("Helvetica", 10)
        for s in S:
            duty = float((s.get("totals") or {}).get("duty", 0.0))
            tot  = float((s.get("totals") or {}).get("total_landed", 0.0))
            c.drawString(0.75*_inch, y, f"• {s.get('name','Scenario')}: Duty ${duty:,.2f}, Total ${tot:,.2f}")
            y -= 0.16*_inch
            if y < 1.0*_inch:
                c.showPage(); w, h = _letter; y = h - 1.0*_inch
    ...
    if bool(brand.get("show_footer", True)):
        ...
        c.setFont("Helvetica-Oblique", 9); c.drawCentredString(w/2, 0.5*_inch, text)
...
def generate_legacy(
    ...
    total_landed = (
        float(merch_value) + float(freight) + float(insurance) +
        duty + float(brokerage) + float(other_fees)
    )
    ...
    if include:
        ...
        if scenario_alt_origin:
            try:
                alt_rate = float(scenario_alt_tariff_rate) if scenario_alt_tariff_rate is not None else float(tariff_rate or 0.0)
            except Exception:
...
def pdf_selftest():
    if ENABLE_PAYWALL:
        return HTMLResponse('<h2>Purchase Required</h2><p>Please buy a report on <a href="/pricing">/pricing</a>.</p>', status_code=402)
    ...
    payload = {
        ...
        "tariff_rate": 0.25,
        "totals": {
            "merch_value": 5000, "freight": 300, "insurance": 25,
            "duty": 1400, "brokerage": 75, "other_fees": 50, "total_landed": 6850
        },
...
def hello_pdf():
    ...
    c = _rl_canvas.Canvas(buf, pagesize=_letter)
    w, h = _letter
    c.setFont("Helvetica-Bold", 18); c.drawString(72, h-72, "Hello from ClearTariff")
    c.setFont("Helvetica", 11); c.drawString(72, h-96, "If you see this, PDF rendering works.")
    c.showPage(); c.save(); buf.seek(0)
    b = buf.getvalue()
...
async def generate_amazon(
    ...
    payload = {
        "meta": {
            "asin": asin, "sku": sku, "product_name": product_name, "hs_code": hs_code,
            "market": market, "country_of_origin": country_of_origin, "email": email,
            "buyer": buyer, "seller": seller, "usd_fx_note": usd_fx_note, "notes": notes,
            "date_str": datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"),
        },
        "numbers": {
            "duty_rate_pct": duty_rate_pct, "units": units, "unit_value_usd": unit_value_usd,
            "freight_usd": freight_usd, "insurance_usd": insurance_usd, "other_costs_usd": other_costs_usd,
